...
void CBlowFish::Encrypt(unsigned char* buf2, size_t n, int iMode)
{
	//Check the buffer's length - should be > 0 and multiple of 8
	if((n==0)||(n%8!=0))
		throw exception("Incorrect buffer length");
	
	std::vector<unsigned char> bufV( n );
	for( int i = 0; i < n; ++i )
	{
		bufV[i] = buf2[bufV.size() - i - 1];
	}

	unsigned char * buf = &bufV[0];
...
//Returns false if n is multiple of 8
void CBlowFish::Decrypt(unsigned char* buf2, size_t n, int iMode)
{
	//Check the buffer's length - should be > 0 and multiple of 8
	if((n==0)||(n%8!=0))
		throw exception("Incorrect buffer length");
