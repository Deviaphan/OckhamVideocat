...
void CScrollHelper::SetDisplaySize( int displayWidth, int displayHeight )
{
	...
	if( m_attachWnd && ::IsWindow( m_attachWnd->m_hWnd ) )
		UpdateScrollInfo();
}
...
void CScrollHelper::SetPageSize( int cx, int cy )
{
	m_pageSize.cx = cx;
	m_pageSize.cy = cy;
}
...
void CScrollHelper::ScrollToPositonY( int y )
{
	m_scrollPos.cy = y;
	m_attachWnd->SetScrollPos( SB_VERT, y, TRUE );
