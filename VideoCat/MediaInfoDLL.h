...
/*-------------------------------------------------------------------------*/
extern std::wstring g_libraryPath;
...
extern "C"
{
    ...
    #include <gmodule.h>
    static GModule* MediaInfo_Module = nullptr;
#elif defined(_WIN32) || defined(WIN32)
    #include <windows.h>
    static HMODULE MediaInfo_Module = nullptr;
#else
    #ifdef MACOSX
    ...
    #include <dlfcn.h>
    static void* MediaInfo_Module = nullptr;
#endif
...
#define MEDIAINFOLIST_ASSIGN(_Name,_Name2) \
    ...
    static size_t MediaInfoDLL_Load()
    {
		...
		if( g_libraryPath.empty() )
			g_libraryPath = MEDIAINFODLL_NAME;

    /* Load library */
    #ifdef MEDIAINFO_GLIBC
        MediaInfo_Module = g_module_open(MEDIAINFODLL_NAME, G_MODULE_BIND_LAZY);
    #elif defined(_WIN32) || defined(WIN32)
        MediaInfo_Module = LoadLibrary( g_libraryPath.c_str() );
    #else
        #ifdef MACOSX
            ...
            if (!MediaInfo_Module)
            {
                ...
                CFURLRef app_path_url = CFURLCreateCopyDeletingLastPathComponent(nullptr, app_url);

                CFStringRef app_path = CFURLCopyFileSystemPath(app_path_url, kCFURLPOSIXPathStyle);

                CFMutableStringRef mut_app_path = CFStringCreateMutableCopy(nullptr, nullptr, app_path);
...
#define MEDIAINFO_TEST_STRING_STATIC \
    ...
    {
    public :
        MediaInfo()        {if (!MediaInfo_Module) MediaInfoDLL_Load(); if (!MediaInfo_Module) {Handle = nullptr; return;}; Handle = MediaInfo_New();};
        ...
        static String Option_Static(const String &Option, const String &Value = __T(""))  {if (!MediaInfo_Module) MediaInfoDLL_Load(); MEDIAINFO_TEST_STRING_STATIC; return MediaInfo_Option(nullptr, Option.c_str(), Value.c_str());};
    ...
    {
    public :
        MediaInfoList()        {MediaInfoDLL_Load(); if (!MediaInfoDLL_IsLoaded()) {Handle = nullptr; return;}; Handle = MediaInfoList_New();};
        ...
        static String Option_Static(const String &Option, const String &Value = __T(""))  {MEDIAINFO_TEST_STRING_STATIC; return MediaInfoList_Option(nullptr, Option.c_str(), Value.c_str());};
